...
SELECT `t`.`Nickname`, IIF((
        ...
        ORDER BY `t2`.`Id`) IS NULL, #0100-01-01#, (
        SELECT TOP 1 `t2`.`IssueDate`
        FROM `Tags` AS `t2`
        WHERE `t2`.`GearNickName` = `t`.`FullName`
...
SELECT `s`.`Name`, (
    ...
    WHERE 'Marcus' IN (
        SELECT `t4`.`Nickname`
        FROM (
            SELECT `t5`.`Nickname`, `t5`.`SquadId`, `t5`.`AssignedCityName`, `t5`.`CityOfBirthName`, `t5`.`FullName`, `t5`.`HasSoulPatch`, `t5`.`LeaderNickname`, `t5`.`LeaderSquadId`, `t5`.`Rank`, `t5`.`Discriminator`
            FROM (
                SELECT `g1`.`Nickname`, `g1`.`SquadId`, `g1`.`AssignedCityName`, `g1`.`CityOfBirthName`, `g1`.`FullName`, `g1`.`HasSoulPatch`, `g1`.`LeaderNickname`, `g1`.`LeaderSquadId`, `g1`.`Rank`, 'Gear' AS `Discriminator`
                FROM `Gears` AS `g1`
                UNION ALL
            ...
            UNION ALL
            SELECT `t6`.`Nickname`, `t6`.`SquadId`, `t6`.`AssignedCityName`, `t6`.`CityOfBirthName`, `t6`.`FullName`, `t6`.`HasSoulPatch`, `t6`.`LeaderNickname`, `t6`.`LeaderSquadId`, `t6`.`Rank`, `t6`.`Discriminator`
            FROM (
